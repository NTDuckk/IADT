...
class PromptComposer(nn.Module):
    """
    Compose text prompts for CLIP text encoder.

    - simplified:  "A photo of a person"
    - composed (PromptSG): "A photo of a X person" where X is replaced by s*
    - composed (IADT): "A photo of a X person with X X ... X" (1 + K placeholders)
        + first X  -> subject token s*
        + next K X -> attribute tokens a*_1..a*_K
    """
    def __init__(self, clip_model, prompt_mode: str, num_attr_tokens: int = 0):
        super().__init__()
        self.prompt_mode = prompt_mode
        self.num_attr_tokens = int(num_attr_tokens)
        self.token_embedding = clip_model.token_embedding
        self.dtype = clip_model.dtype

        if self.num_attr_tokens > 0:
            attrs = " ".join(["X"] * self.num_attr_tokens)
            self.composed_str = f"A photo of a X person with {attrs}"
        else:
            self.composed_str = "A photo of a X person"

        self.simplified_str = "A photo of a person"

        # register buffers once (empty => init on first forward)
    ...
    def _ensure_tokenization(self):
        ...
        x_pos = (tokenized_composed[0] == x_token_id).nonzero(as_tuple=False).view(-1)
        if x_pos.numel() == 0:
            raise ValueError("Cannot locate placeholder token 'X' in composed prompt")

        expected = 1 + self.num_attr_tokens
        if x_pos.numel() != expected:
            raise ValueError(
                f"Composed prompt expected {expected} 'X' placeholders but got {x_pos.numel()} "
                f"(prompt='{self.composed_str}')"
            )

        self.tokenized_composed = tokenized_composed
        self.tokenized_simplified = tokenized_simplified
        self.x_pos_all = [int(v.item()) for v in x_pos]
    ...
    def forward(self, s_star: torch.Tensor, attr_tokens=None):
        """
        s_star: (B, D)
        attr_tokens: (B, K, D) or None
        """
        ...
        prompts = self.embed_composed.expand(b, -1, -1).clone()
        s_star = s_star.to(dtype=prompts.dtype)
        prompts[:, self.x_pos_all[0], :] = s_star

        if self.num_attr_tokens > 0:
            if attr_tokens is None:
                raise ValueError("IADT prompt requires attr_tokens but got None")
            if attr_tokens.size(1) != self.num_attr_tokens:
                raise ValueError(
                    f"attr_tokens K mismatch: expected {self.num_attr_tokens} but got {attr_tokens.size(1)}"
                )
            attr_tokens = attr_tokens.to(dtype=prompts.dtype)
            for j in range(self.num_attr_tokens):
                prompts[:, self.x_pos_all[1 + j], :] = attr_tokens[:, j, :]
...
class AttrTokenBlock(nn.Module):
    """A small cross-attention + FFN block for attribute tokenization (IADT-style)."""
...
def orthogonal_token_loss(tokens: torch.Tensor, eps: float = 1e-6):
...
class AttributeTokenizer(nn.Module):
    ...
    - learnable queries X (num_queries)
    - cross-attn from queries to patch tokens (num_layers blocks)
    - top-k filtering by cosine similarity with global embedding v
    - mapping network f_A to token embedding space
    """
    def __init__(
        self,
        embed_dim: int = 512,
        ...
        self.query_embed = nn.Parameter(torch.empty(self.num_queries, embed_dim))
        trunc_normal_(self.query_embed, std=0.02)

        self.blocks = nn.ModuleList([
            AttrTokenBlock(embed_dim=embed_dim, num_heads=num_heads, mlp_ratio=4.0, dropout=dropout)
            for _ in range(int(num_layers))
        ])

        # f_A mapping (3-layer MLP + BN), same structure as inversion network
        self.attr_mapper = InversionNetwork(dim=embed_dim)
    ...
    def forward(self, patch_tokens: torch.Tensor, v_global: torch.Tensor):
        ...
        # top-k filtering by cosine similarity with global embedding
        ...
        attr_flat = attr.reshape(B * topk, D)
        attr_mapped = self.attr_mapper(attr_flat).reshape(B, topk, D)
...
class MultimodalInteractionModule(nn.Module):
    ...
    def __init__(
        ...
        eps: float = 1e-6,
        act_layer=QuickGELU,
        include_cls_in_kv: bool = False,
        reweight_cls: bool = False,
    ):
        super().__init__()
        ...
        self.eps = eps
        self.include_cls_in_kv = include_cls_in_kv
        self.reweight_cls = reweight_cls
    ...
    def forward(self, text_tokens_full, eot_idx, patch_tokens, cls_token, return_cls_states=False):
        ...
        if self.include_cls_in_kv:
            kv_tokens = torch.cat([cls_token, patch_tokens], dim=1)      # (B, 1+M, D)
            kv_tokens = self.kv_ln(kv_tokens)
            kv_for_attn = kv_tokens
        else:
            kv_for_attn = self.kv_ln(patch_tokens)                       # (B, M, D)
        ...
        _, attn_w = self.cross_attn(q, kv_for_attn, kv_for_attn, need_weights=True)  # (B,H,Nq,Nkv)

        # attn_map modes
        attn_map_all = self._make_attn_map(attn_w, eot_idx=eot_idx)      # (B,1,Nkv)

        if self.include_cls_in_kv:
            cls_w = attn_map_all[:, :, :1]                               # (B,1,1)
            patch_w = attn_map_all[:, :, 1:]                             # (B,1,M)

            # reweight patches
            patch_rw = self._reweight_patches(patch_tokens, patch_w)     # (B,M,D)

            # optionally reweight CLS token too
            if self.reweight_cls:
                w = cls_w.transpose(1, 2)                                # (B,1,1)
                if self.reweight == "mul_mean1":
                    scale = w * (M + 1)
                    cls_rw = cls_token * scale
                elif self.reweight == "mul":
                    cls_rw = cls_token * w
                elif self.reweight == "residual":
                    scale = w * (M + 1)
                    cls_rw = cls_token * (1.0 + scale)
                else:
                    raise ValueError(f"Unknown reweight mode: {self.reweight}")
            else:
                cls_rw = cls_token

            attn_map = patch_w  # keep backward-compatible (B,1,M)
        else:
            attn_map = attn_map_all                                     # (B,1,M)
            patch_rw = self._reweight_patches(patch_tokens, attn_map)    # (B,M,D)
            cls_rw = cls_token

        # seq for post blocks
        seq = torch.cat([cls_rw, patch_rw], dim=1)  # (B,1+M,D)
...
class PromptSGModel(nn.Module):
    def __init__(self, num_classes, camera_num, view_num, cfg):
        ...
        # PromptSG / IADT modules
        self.enable_iadt = bool(getattr(cfg.MODEL.PROMPTSG, "ENABLE_IADT", getattr(cfg.MODEL.PROMPTSG, "IADT_ENABLED", False)))
        self.iadt_num_queries = int(getattr(cfg.MODEL.PROMPTSG, "IADT_NUM_QUERIES", 64))
        self.iadt_topk = int(getattr(cfg.MODEL.PROMPTSG, "IADT_TOPK", 16))
        self.iadt_attr_patch_source = str(getattr(cfg.MODEL.PROMPTSG, "IADT_ATTR_PATCH_SOURCE", "proj")).lower()

        # prompt composer: if IADT enabled -> add K attribute placeholders
        self.prompt_composer = PromptComposer(
            clip_model,
            cfg.MODEL.PROMPTSG.PROMPT_MODE,
            num_attr_tokens=(self.iadt_topk if self.enable_iadt else 0),
        )

        # inversion network f_S (PromptSG-style)
        self.inversion = InversionNetwork(dim=512)  # inversion always takes 512-d

        # IADT attribute tokenizer (learnable queries + top-k)
        if self.enable_iadt:
            self.attr_tokenizer = AttributeTokenizer(
                embed_dim=512,
                num_queries=self.iadt_num_queries,
                topk=self.iadt_topk,
                num_layers=int(getattr(cfg.MODEL.PROMPTSG, "IADT_ATTR_LAYERS", 2)),
                num_heads=int(getattr(cfg.MODEL.PROMPTSG, "IADT_ATTR_HEADS", cfg.MODEL.PROMPTSG.CROSS_ATTN_HEADS)),
                dropout=float(getattr(cfg.MODEL.PROMPTSG, "IADT_ATTR_DROPOUT", 0.0)),
            )
        else:
            self.attr_tokenizer = None
...
# Projections for RN50 (define in __init__ to ensure optimizer sees params)
        ...
        # Extra projections for IADT patch source "final"/"both"
        if self.model_name == "ViT-B-16":
            self.vit_final_patch_proj = nn.Linear(768, 512)
            self.vit_final_patch_proj.apply(weights_init_kaiming)
        if self.model_name == "RN50":
            self.resnet_final_patch_proj = nn.Linear(2048, 512)
            self.resnet_final_patch_proj.apply(weights_init_kaiming)
...
# MIM (explicit attention + full/eot query + attn_map modes)
        self.mim = MultimodalInteractionModule(
            ...
            include_cls_in_kv=getattr(cfg.MODEL.PROMPTSG, "INCLUDE_CLS_IN_KV", self.enable_iadt),
            reweight_cls=getattr(cfg.MODEL.PROMPTSG, "REWEIGHT_CLS", False),
            act_layer=QuickGELU,
        )
    ...
    def forward(self, x=None, label=None, get_image=False, get_text=False, cam_label=None, view_label=None):
        ...
        if get_text:
            if self.prompt_mode == 'simplified':
                ...
                ortho_loss = None
                if self.enable_iadt:
                    # build attribute patch tokens (proj/final/both) BEFORE composing prompt
                    if self.model_name == 'ViT-B-16':
                        patches_proj = features_proj[:, 1:]                     # (B,M,512)
                        patches_final = self.vit_final_patch_proj(features_final[:, 1:])  # (B,M,512)
                    else:  # RN50
                        if len(features_proj) > 1:
                            b, c, h, w = features_proj[1].shape  # c=512
                            patches_proj = features_proj[1].view(b, c, -1).permute(0, 2, 1)  # (B,M,512)
                        else:
                            patches_proj = None
                        b, c, h, w = features_final.shape  # c=2048
                        patches_final = features_final.view(b, c, -1).permute(0, 2, 1)       # (B,M,2048)
                        patches_final = self.resnet_final_patch_proj(patches_final)          # (B,M,512)
                        if patches_proj is None:
                            patches_proj = patches_final

                    src = self.iadt_attr_patch_source
                    if src == "proj":
                        patches_attr = patches_proj
                    elif src == "final":
                        patches_attr = patches_final
                    elif src == "both":
                        patches_attr = torch.cat([patches_proj, patches_final], dim=1)
                    else:
                        raise ValueError(f"Unknown IADT_ATTR_PATCH_SOURCE: {src}")

                    attr_tokens, ortho_loss = self.attr_tokenizer(patches_attr, v)
                    prompts, tokenized = self.prompt_composer(s_star, attr_tokens)
                else:
        ...
        if self.prompt_mode == 'simplified':
            ...
            # IMPORTANT: no torch.no_grad(), no detach -> allow gradient to flow back to inversion / attr tokenizer
        ...
        if self.training:
            ...
            if self.enable_iadt:
                return [cls_score, cls_score_proj], triplet_feats, v, text_feat, ortho_loss
        ...
        if self.prompt_mode == 'simplified':
            ...
            if self.enable_iadt:
                if self.model_name == 'ViT-B-16':
                    ...
                    patches_final = self.vit_final_patch_proj(features_final[:, 1:])
                else:
                    patches_proj = patches
                    b, c, h, w = features_final.shape
                    patches_final = features_final.view(b, c, -1).permute(0, 2, 1)
                    patches_final = self.resnet_final_patch_proj(patches_final)

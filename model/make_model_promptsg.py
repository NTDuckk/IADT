...
from .attribute_tokenizer import AttributeTokenizer
...
class PromptComposer(nn.Module):
    ...
    - composed (IADT): "photo of X person with A A ... A" (1 + K placeholders)
        + first X  -> subject token s*
        + next K A -> attribute tokens a*_1..a*_K
    """
    def __init__(self, clip_model, prompt_mode: str, num_attr_tokens: int = 0):
        ...
        if self.num_attr_tokens > 0:
            attrs = " ".join(["A"] * self.num_attr_tokens)
            self.composed_str = f"photo of X person with {attrs}"
        else:
            self.composed_str = "photo of X person"
    ...
    def _ensure_tokenization(self):
        ...
        if x_pos.numel() == 0:
            raise ValueError("Cannot locate subject placeholder token 'X' in composed prompt")
        if x_pos.numel() != 1:
            raise ValueError(f"Subject placeholder 'X' must appear exactly once, got {x_pos.numel()} (prompt='{self.composed_str}')")

        tokenized_a = clip_module.tokenize(["A"]).to(dev)
        a_token_id = tokenized_a[0, 1].item()
        a_pos = (tokenized_composed[0] == a_token_id).nonzero(as_tuple=False).view(-1)
        if self.num_attr_tokens > 0:
            if a_pos.numel() == 0:
                raise ValueError("Cannot locate attribute placeholder token 'A' in composed prompt")
            if a_pos.numel() != self.num_attr_tokens:
                raise ValueError(
                    f"Composed prompt expected {self.num_attr_tokens} 'A' placeholders but got {a_pos.numel()} "
                    f"(prompt='{self.composed_str}')"
                )
        else:
            # allow 0
            if a_pos.numel() != 0:
                raise ValueError(f"Prompt has unexpected 'A' tokens but num_attr_tokens=0 (got {a_pos.numel()})")

        self.tokenized_composed = tokenized_composed
        self.tokenized_simplified = tokenized_simplified
        self.x_pos = int(x_pos.item())
        self.a_pos_all = [int(v.item()) for v in a_pos]
    ...
    def forward(self, s_star: torch.Tensor, attr_tokens=None):
        ...
        if self.num_attr_tokens > 0:
            ...
            for j in range(self.num_attr_tokens):
                prompts[:, self.a_pos_all[j], :] = attr_tokens[:, j, :]

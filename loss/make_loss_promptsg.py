...
def make_loss(cfg, num_classes):
    ...
    if sampler == 'softmax':
        ...
        def loss_func(score, feat, target, target_cam, image_feat=None, text_feat=None, ortho_loss=None):
            ...
            if cfg.MODEL.IF_LABELSMOOTH == 'on':
                ...
                enable_iadt = bool(getattr(cfg.MODEL.PROMPTSG, 'ENABLE_IADT', getattr(cfg.MODEL.PROMPTSG, 'IADT_ENABLED', False)))
                lambda_supcon = getattr(cfg.MODEL.PROMPTSG, 'IADT_LAMBDA_SUPCON', (0.1 if enable_iadt else cfg.MODEL.PROMPTSG.LAMBDA_SUPCON))
                lambda_ortho = getattr(cfg.MODEL.PROMPTSG, 'IADT_LAMBDA_ORTHO', 1.0)
                ...
                if (image_feat is not None) and (text_feat is not None):
                    if (prompt_mode != "simplified") or enable_supcon_in_simplified:
                        ...
                        total_loss = total_loss + lambda_supcon * SUPCON_LOSS
                
                # IADT: orthogonal regularization on attribute tokens
                ORTHO_LOSS = torch.tensor(0.0, device=ID_LOSS.device)
                if enable_iadt and (ortho_loss is not None):
